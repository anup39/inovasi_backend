...
BEGIN
    ...
    mill_point := trim((query_params::jsonb) ->> 'mill_point'));
    statusplot := trim((query_params::jsonb) ->> 'status_of_plot'));
    SELECT INTO mvt ST_AsMVT(tile, 'function_zxy_query_app_agriplot_by_radius_and_status', 4096, 'geom') FROM (
        SELECT
            ...
            app_agriplot.risk_assess, app_agriplot.ghg_luc, app_agriplot.status_of_plot, app_agriplot.millideq,app_agriplot.def_free,app_agriplot.compliance
        FROM public.app_agriplot 
        WHERE public.app_agriplot.status_of_plot::text = statusplot AND ST_DWithin(public.app_agriplot.geom, ST_GeomFromText(mill_point, 4326), radius::double precision)
    ) AS tile WHERE geom IS NOT NULL;
    RETURN mvt;
END
$$
